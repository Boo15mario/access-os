...
then 
  ...
  echo "Failed to enable xrdp. It will need to be manually enabled later."
fi
...
cd
echo "Downloading .xinitrc"
curl -O "https://boo15mario.boo/scripts/access-os/system-files/xinitrc"
if [ $? -eq 0 ] 
then 
  echo "Successfully created file" 
else 
  echo "The installation of access-os failed. Reason: Failed to retrieve .xinitrc. Command returned exit code $?. Please try the installation again." >&2
  exit 1
fi
mv -v xinitrc .xinitrc
